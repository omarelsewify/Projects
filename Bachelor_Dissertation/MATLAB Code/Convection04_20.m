...
while time < tfinal
    for i=2:res-1
        for j=2:res-1
            % Bottom Left Corner
            if (i == 1 && j == 1)
                phi(i,j) = 0.25*(phi_old(i-1,j)+phi_old(i,j-1)+phi_old(i+1,j)+phi_old(i,j+1));
            ...
            elseif (i == res && j == res)
                ...
                % Bottom Edge
            elseif (j == 1)            
                phi_old(i,j-1) = (k/(k-2*gs))*phi_old(i,j+1) - (2*gs*h/(k-2*gs*h))*phi_old(i,j);
                phi(i,j) = phi_old(i,j)+Fo*(phi_old(i,j+1)-4*phi_old(i,j)+phi_old(i,j-1)+phi_old(i+1,j)+phi_old(i-1,j));
                
                % Right Edge
            elseif (j ~= res && i == res)
                
                phi_old(i+1,j) = (k/(k-2*gs))*phi_old(i-1,j) - (2*gs*h/(k-2*gs*h))*phi_old(i,j);
                phi(i,j) = phi_old(i,j)+Fo*(phi_old(i,j+1)-4*phi_old(i,j)+phi_old(i,j-1)+phi_old(i+1,j)+phi_old(i-1,j));
