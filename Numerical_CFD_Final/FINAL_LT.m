...
while time < Inf
    ...
    for i = 2:nx-1
        for j = 2:ny-1
            ...
            c = sqrt(gam*V(i,j,4)/V(i,j,1));
            max_c(i,j) = max(abs(V(i,j,2)-c),abs(V(i,j,2)+c));

            dtL = 0.95*(area(i,j))/max_c(i,j);

            W(i,j,:) = Wi - (dtL/area(i,j))*(Fij + Fik + Fil + Fim);
            V(i,j,:) = W_to_V(W(i,j,:));
    ...
    for i = 1
        for j = 2:ny-1
            ...
            W(i,j,:) = Wi - (dtL/area(i,j))*(Fij + Fik + Fim + Fi_inf);
            V(i,j,:) = W_to_V(W(i,j,:));
    ...
    for i = nx
        for j = 2:ny-1
            ...
            W(i,j,:) = Wi - (dtL/area(i,j))*(Fik + Fil + Fim + Fi_inf);
            V(i,j,:) = W_to_V(W(i,j,:));
    ...
    for i = 2:nx-1
        for j = 1
            ...
            W(i,j,:) = Wi - (dtL/area(i,j))*(Fij + Fil + Fik+ Fi_wall);
            V(i,j,:) = W_to_V(W(i,j,:));
    ...
    for i = 2:nx-1
        for j = ny
            ...
            W(i,j,:) = Wi - (dtL/area(i,j))*(Fij + Fil + Fim+ Fi_wall);
            V(i,j,:) = W_to_V(W(i,j,:));
